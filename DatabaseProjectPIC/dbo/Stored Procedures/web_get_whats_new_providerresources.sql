...
CREATE PROCEDURE [dbo].[web_get_whats_new_providerresources] (
	@location_id	int,
	@lob_id		int,
	@plob		varchar(10),
	@max_items	int)
...
INSERT INTO #WhatsNew
	SELECT whats_new_id, eff_date, story_name, file_name, priority, 0 AS totalnewcount FROM web_whats_new_pic WHERE archived_flag=0 AND providerresource_flag=1 AND active_flag = 1 AND (all_flag = 1 OR (lob_id = @lob_id AND multiple_payor_flag = 0 AND location_flag = 0));
...
INSERT INTO #WhatsNew
	...
	WHERE 
	archived_flag=0 AND providerresource_flag=1 AND active_flag = 1
	AND lob_id = @lob_id
